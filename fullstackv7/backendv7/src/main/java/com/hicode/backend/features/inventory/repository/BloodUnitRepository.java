package com.hicode.backend.features.inventory.repository;
...
public interface BloodUnitRepository extends JpaRepository<BloodUnit, String> {
     ...
     */
    @Query("SELECT new com.hicode.backend.features.admin.dto.InventorySummary(bu.bloodType, COUNT(bu.id), SUM(bu.volumeMl)) " +
            "FROM BloodUnit bu " +
